...
void ProcessMove(const string& line, Position knots[10], vector<Position>& tail_positions) {
    ...
    while (spaces > 0) {
        ...
        for (int i = 0; i < 9; ++i) {
            ProcessTailMove(knots[i], knots[i+1]);
        }
...
int main(int argc, char* argv[]) {
    ...
    while (getline(input, line)) {
        ProcessMove(line, knots, tail_positions);
    }
